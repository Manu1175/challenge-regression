...
class Immo_Preprocessing:
    ...
    def eliminate_outliers(self):
        ...
        self.df = self.df[
            (self.df['price'] >= 50000) & (self.df['price'] <= 1_000_000) &
            (self.df['habitablesurface'] >= 15) & (self.df['habitablesurface'] <= 600) &
            (self.df['bedroomcount'] <= 20) 
        ]
    ...
    def add_distance_from_province_capital(self):
        province_capitals = {
            'Antwerp': (51.2194, 4.4025),
            'East Flanders': (51.0543, 3.7174),  # Ghent
            'Flemish Brabant': (50.8798, 4.7005), # Leuven
            'Hainaut': (50.4114, 4.4446),         # Charleroi
            'Liège': (50.6326, 5.5797),
            'Limburg': (50.9303, 5.3323),         # Hasselt
            'Luxembourg': (49.8153, 5.4110),      # Arlon
            'Namur': (50.4669, 4.8679),
            'Walloon Brabant': (50.6764, 4.6089), # Wavre
            'West Flanders': (51.2093, 3.2247),   # Bruges
            'Brussels-Capital': (50.8503, 4.3517),
            }
        def distance_to_province_capital(row):
            lat, lon = row['latitude'], row['longitude']
            province = row['province']
            if province in province_capitals:
                capital_lat, capital_lon = province_capitals[province]
                # Calculate the distance using haversine formula
                # haversine returns distance in kilometers by default
                distance = haversine((lat, lon), (capital_lat, capital_lon), unit=Unit.KILOMETERS)
                return distance
            else:
                return None  # or a default value

        self.df['distance_to_province_capital'] = self.df.apply(distance_to_province_capital, axis=1)



    def add_region_column_kmeans(self):
        # 1. Scale lat/lon
        scaler = StandardScaler()
        coords_scaled = scaler.fit_transform(self.df[['latitude', 'longitude']])

        # 2. Fit KMeans
        k = 30  # you can adjust this
        kmeans = KMeans(n_clusters=k, random_state=42, n_init='auto')
        self.df['region_cluster'] = kmeans.fit_predict(coords_scaled)

        # 3. Optional: Check distribution
        print(self.df['region_cluster'].value_counts())

        # 4. Optional: Visual inspection (sanity check)
        import matplotlib.pyplot as plt
        plt.figure(figsize=(8, 6))
        plt.scatter(self.df['longitude'], self.df['latitude'], c=self.df['region_cluster'], cmap='tab20', s=5)
        plt.title('Region Clusters via KMeans')
    ...
    def data_pre_processing(self):
        ...
        self.add_distance_from_province_capital()
        self.add_region_column_kmeans()
